...
- **CAL (cryptography abstraction layer)** lives in `cal/` and plugs in “Cryptographic Functionality Providers” (CFP).
    - We need cryptographic operations available in Libtropic to securely communicate with the TROPIC01 on Layer 3 (via the Secure Channel Session, which is encrypted).
...
- **L3 state** (Secure Channel Session status, an L3 buffer, and a pointer to your CAL/CFP context at `h.l3.crypto_ctx`).
...
- If `lt_init(&h)` fails, do **not** call `lt_deinit(&h)` (cleanup is handled internally).
- As a general C best practice, initialize the handle to zero before filling in pointers (e.g., `lt_handle_t h = {0};`).
...
5. Use L2 requests and/or establish a Secure Channel Session for L3.
