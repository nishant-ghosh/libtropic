...
if [ -z "$STLINK_SERIAL_NUMBER" ]; then
    ...
    openocd -f "$SCRIPT_DIR/nucleo-u5xx.cfg" -c "program $1 verify reset exit"
else
    echo "Using STLink serial number: $STLINK_SERIAL_NUMBER"
    openocd -f "$SCRIPT_DIR/nucleo-u5xx.cfg" -c "adapter serial $STLINK_SERIAL_NUMBER" -c "program $1 verify reset exit"
fi