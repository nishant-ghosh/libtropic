 ...
 * @brief Provision admin key (SH1) using SH0
...
{
    LT_LOG_INFO("----------------------------------------------");
    LT_LOG_INFO("lt_test_ire_provision_admin_key()");
    LT_LOG_INFO("----------------------------------------------");
    ...
    // Write admin pairing key into slot 1
    LT_LOG_INFO("Writing to pairing key slot %" PRIu8 "...", 1);
    LT_TEST_ASSERT(LT_OK, lt_print_bytes(pub_keys[1], TR01_SHIPUB_LEN, print_buff, PRINT_BUFF_SIZE));
    LT_LOG_INFO("%s", print_buff);
    LT_TEST_ASSERT(LT_OK, lt_pairing_key_write(h, pub_keys[1], 1));
    LT_LOG_INFO();
    LT_LOG_LINE();

    // Read pairing keys 0-1 and check value
    for (uint8_t i = TR01_PAIRING_KEY_SLOT_INDEX_0; i <= TR01_PAIRING_KEY_SLOT_INDEX_1; i++) {
    ...
    // Test secure session with key slot 1
    LT_LOG_INFO("Starting Secure Session with key %" PRIu8 "...", 1);
    LT_TEST_ASSERT(LT_OK, lt_verify_chip_and_start_secure_session(h, priv_keys[1], pub_keys[1], 1));

    LT_LOG_INFO("Aborting Secure Session with slot %" PRIu8 "...", 1);
    LT_TEST_ASSERT(LT_OK, lt_session_abort(h));
    LT_LOG_LINE();
