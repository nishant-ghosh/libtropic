...
typedef enum lt_asn1der_crop_kind_t {
    LT_ASN1DER_CROP_SUFFIX,
    LT_ASN1DER_CROP_PREFIX
} lt_asn1der_crop_kind_t;
 ...
 *                          LT_ASN1DER_CROP_SUFFIX - Crop Suffix , copy to "buf" from 0 to "buf_len" -
 * 1 LT_ASN1DER_CROP_PREFIX - Crop Prefix, copy to "buf" from N - buf_len to N - 1 where N is length of
 ...
 *                      LT_CERT_UNSUPPORTED if the ASN1 stream contains features unsupported by this
 * parser LT_CERT_ITEM_NOT_FOUND if OBJECT_IDENTIFIER with "obj_id" value was not found!
 */
